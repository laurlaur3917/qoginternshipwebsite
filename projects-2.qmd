...
ggplot() + geom_sf(data= year2016 %>% filter(dominantparty=="Republican"), aes(geometry=geometry, fill=republicancolor)) + scale_fill_identity(name = "Republican Vote %", labels = c("90%-100%", "80%-90%", "70%-80%", "60%-70%", "50%-60%"), guide="legend") + new_scale_fill() + geom_sf(data= year2016 %>% filter(dominantparty=="Democrat"), aes(geometry=geometry, fill=democratcolor)) + scale_fill_identity(name= "Democrat Vote %", labels=c("90%-100%", "80%-90%", "70%-80%", "60%-70%", "50%-60%"), guide="legend") + ggthemes::theme_map() + theme(legend.position = "right")
...
```{r}
ggplot() + geom_sf(data= year1916 %>% filter(dominantparty=="Republican"), aes(geometry=geometry, fill=republicancolor)) + scale_fill_identity(name = "Republican Vote %", labels = c("90%-100%", "80%-90%", "70%-80%", "60%-70%", "50%-60%", "NA"), guide="legend") + new_scale_fill() +  geom_sf(data= year1916 %>% filter(dominantparty=="Democrat"), aes(geometry=geometry, fill=democratcolor)) + scale_fill_identity(name = "Democrat Vote %", labels = c("90%-100%", "80%-90%", "70%-80%", "60%-70%", "50%-60%", "NA"), guide="legend") + ggthemes::theme_map() + theme(legend.position = "right")
...
g<-ggplot() + geom_sf(data= mapcolors %>% filter(dominantparty=="Republican"), aes(geometry=geometry, fill=republicancolor)) + scale_fill_identity(name = "Republican Vote %", labels = c("90%-100%", "80%-90%", "70%-80%", "60%-70%", "50%-60%"), guide="legend") + new_scale_fill() +  geom_sf(data= mapcolors %>% filter(dominantparty=="Democrat"), aes(geometry=geometry, fill=democratcolor)) + scale_fill_identity(name = "Democrat Vote %", labels = c("90%-100%", "80%-90%", "70%-80%", "60%-70%", "50%-60%"), guide="legend") + ggforce::facet_wrap_paginate(~election_year, ncol=1, nrow=1, page=i) + coord_sf(xlim = c(-140, -60)) + labs(title=paste("Election Year:", current_year)) + theme(plot.title = element_text(hjust = 0.5, size = 16, face = "bold"), legend.position = "right", panel.background = element_blank(), axis.text = element_blank(),axis.ticks = element_blank(), axis.title = element_blank(), panel.grid = element_blank())
...
```{r}
#library(highcharter)
#dataanimated<- mergedelectiondata %>%
  #mutate(value = ifelse(republican_raw_votes_2perc>0.5, republican_raw_votes_2perc, -democratic_raw_votes_2perc)) %>% mutate(value=ifelse(is.na(value), NA, value)) %>% select(election_year, value, GEO_ID)
...
#hc <- highchart(type = "map") %>% 
  #hc_add_series(
    #data = ds,
    #name = "US election results from 1916-2020",
    #mapData = us_data2,
    #joinBy= "GEO_ID",
    #borderWidth = 0.01
    #) %>% 
  #hc_colorAxis(stops = color_stops()) %>%  
  #hc_title(text = "US election results from 1916-2020") %>% 
  #hc_subtitle(text = "% vote of winning party") %>% 
  #hc_legend(
    #layout = "horizontal",
    #reversed = TRUE,
    #floating = TRUE,
  ...
  #hc_motion(
    #enabled = TRUE,
    #axisLabel = "year",
    #labels = sort(unique(dataanimated$election_year)),
    #series = 0,
    #updateIterval = 50,
    #magnet = list(
      #round = "floor",
