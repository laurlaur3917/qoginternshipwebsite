...
```{r}
mapcolors <- mergedelectiondata %>% mutate(republicancolor = case_when(republican_excess>0.4 & republican_excess<=0.5 ~ "#a70000", republican_excess > 0.3 & republican_excess <= 0.4 ~ "#ff0000", republican_excess > 0.2 & republican_excess <= 0.3 ~ "#ff5252", republican_excess > 0.1 & republican_excess <= 0.2 ~ 	"#ff7b7b", republican_excess >= 0 & republican_excess<=0.1 ~ "#ffbaba", TRUE ~ NA_character_)) %>% mutate(democratcolor= case_when(democrat_excess>0.4 & democrat_excess<=0.5 ~ "#0200b9",democrat_excess > 0.3 & democrat_excess<= 0.4 ~ "#1000ff",democrat_excess > 0.2 & democrat_excess <= 0.3 ~ "#3399ff", democrat_excess > 0.1 & democrat_excess <= 0.2 ~ "#3a80ec", democrat_excess >= 0 & democrat_excess<= 0.1 ~ "#cce0ff", TRUE ~ NA_character_)) 
```
...
#make map
highchart(type = "map") %>%
   hc_add_series_map(df= highchartest, map= Myhcmap, name= "% of vote", value="winningpercentage", joinBy= "GEO_ID", showInLegend = T) %>% hc_tooltip(pointFormat=tt, useHTML=TRUE) %>% hc_colorAxis(min=50,max=100) %>% hc_mapNavigation(enabled = TRUE) %>% hc_title(text= "2016 Election Results") %>% hc_subtitle(text="Winning Party's Share of Vote by County") %>% hc_legend(align="right")

#can i change the border color/width? couldn't find solution 
#can't make double scale unless i made democrat or republican values negative don't want to confuse the viewer. 
